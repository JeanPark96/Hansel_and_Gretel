...
public class drawCanvas extends View {
   ...
   @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
       ...
       // CanvasBitmap= BitmapFactory.decodeResource(getResources(),R.drawable.grid);//res 폴더에 저장된 이미지를 Bitmap으로 만들때 사용함
       // CanvasMap.setImageBitmap(CanvasBitmap);
        //drawBitmap=Bitmap.createBitmap(CanvasBitmap.getWidth(),CanvasBitmap.getHeight(),Bitmap.Config.ALPHA_8);//CanvasBitmap를 그대로 가져오면서 흑백처리 해버림.
