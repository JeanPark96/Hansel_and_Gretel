...
public class ImageDBhelper extends SQLiteOpenHelper {
    ...
    public static final String PATH_LAST_AZIMUTH="path_azimuth";
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS path_image ( "+" row_id INTEGER PRIMARY KEY AUTOINCREMENT,"+"path_available INTEGER,"+" image BLOB,"+"path_azimuth REAL,"+"path_position_x REAL,"+"path_position_y REAL);");
    }
    ...
    public boolean updateDB(int id,int path_available,byte[] image,float path_azimuth, float path_position_x, float path_position_y){
        SQLiteDatabase db=this.getWritableDatabase();
        ContentValues content=new ContentValues();
        content.put(PATH_IMAGE,image);
        content.put(PATH_AVAILABLE_NUM,path_available);
        content.put(PATH_LAST_AZIMUTH, path_azimuth);
    ...
    public ArrayList<ImageInfo> getAllData(){
        ...
        while(res.isAfterLast()==false){
            ...
            float path_azimuth=res.getFloat(3);
            float path_position_x=res.getFloat(4);
            float path_position_y=res.getFloat(5);
            ImageInfo newImage=new ImageInfo(id,path_available,image,path_azimuth,path_position_x,path_position_y);
